...
\begin{enumerate}
	...
	\item Descarte de 8 variables con $>50\%$ nulos y eliminación de ruido estructural (\texttt{HOGAR}, \texttt{N\_REN}).
	...
	\begin{itemize}
		...
		\item \textbf{NIV}: Imputación por \textit{regresión} (R² = 0.0087, aprovechando correlación con \texttt{GRA}).
		\item \textbf{Resto de variables} (\texttt{GRA}, \texttt{P2\_9}, \texttt{P2\_10}, \texttt{P2\_11}, \texttt{P2\_13}, \texttt{P2\_16}): Imputación por \textit{mediana} (distribuciones sesgadas o con outliers).
	\end{itemize}
	\item Aplicación de imputación mediante \texttt{SimpleImputer} (media/mediana) y regresión lineal para \texttt{NIV}.
	\item Normalización con \texttt{MinMaxScaler} para escalar variables a $[0,1]$.
...
Todas las operaciones se realizaron en Python 3.13 usando las librerías \texttt{pandas}, \texttt{numpy}, \texttt{matplotlib}, \texttt{seaborn} y \texttt{scikit-learn}. El código completo se encuentra en \texttt{src/analisis.py}.
...
La Figura~\ref{fig:boxplot_inicial} muestra la distribución inicial de las variables antes de la imputación, revelando una alta concentración de valores faltantes en variables como \texttt{P2\_12} (93.63\%). La imputación por media preservó la simetría de \texttt{EDAD} (media = 32.98, desviación estándar = 21.42), mientras que la regresión aplicada a \texttt{NIV} (R² = 0.0087) redujo ligeramente el sesgo entre media (3.86) y mediana (3), como se observa en la Figura~\ref{fig:boxplot_edad_niv}.
...
Se aplicaron métodos específicos según la naturaleza de cada variable:
\begin{itemize}
	\item \texttt{EDAD}: Imputación por media (distribución simétrica, sesgo < 0.1).
	\item \texttt{NIV}: Imputación por regresión lineal (R² = 0.0087), aprovechando su correlación con \texttt{GRA}.
	\item Resto de variables: Imputación por mediana (distribuciones sesgadas).
\end{itemize}
...
\begin{figure}[H]
	...
	\caption{Comparación EDAD (media) y NIV (regresión)}
...
\begin{enumerate}
	\item Los datos faltantes en variables demográficas (\texttt{EDAD}) son MCAR/MAR y manejables con métodos simples.
	\item Las variables sensibles (\texttt{NIV}) requieren enfoques avanzados (regresión) debido a su naturaleza MNAR.
	\item La imputación preservó las relaciones clave entre variables, permitiendo un análisis válido de factores asociados a la violencia de género en la ENDIREH 2021.
\end{enumerate}

\section{Conclusión}

El análisis de datos faltantes es fundamental. Los hallazgos ENDIREH 2021 revelan: tasas de no respuesta menor al  70\% en preguntas incomodas; outliers en EDAD. La imputación por media para EDAD y regresión para NIV mantienen una simetría y relaciones entre variables.
